    ...
    <div class="container">
        <form class="row #455a64 blue-grey darken-2" method="POST" action="../controllers/controller-signin.php" novalidate>
            <div class="input-field col s12 m6 offset-m3 center-align">
                <input id="enterprise_email" type="email" placeholder="saisissez votre adresse mail" class="validate <?php if (isset($errors['enterprise_email'])) echo 'invalid'; ?>" name="enterprise_email" value="<?= isset($_POST['enterprise_email']) ? htmlspecialchars($_POST['enterprise_email']) : '' ?>" required> <label for="enterprise_email" class="active cyan-text text-lighten-5">Email :</label>
                <span class="helper-text" data-error="<?php echo isset($errors['enterprise_email']) ? $errors['enterprise_email'] : 'Champ obligatoire'; ?>"></span>
            </div>
            <div class="input-field col s12 m6 offset-m3 center-align">
                <input id="enterprise_password"  placeholder="saisissez votre mot de passe" type="password" class="validate <?php if (isset($errors['enterprise_password'])) echo 'invalid'; ?>" name="enterprise_password" placeholder="Votre mot de passe" aria-label="password" aria-describedby="password">
